...
{ ===================== AVL borradores (global y por usuario) ===================== }
  ...
  try
    ...
    begin
      ...
      L.Add(Format(
        ...
        [ NodeId(D), D^.id, Esc(D^.remitente), Esc(D^.asunto), Esc(D^.fecha), Esc(D^.mensaje) ]));
    end;
    ...
    begin
      D := PDraft(T[i]);
      if D = nil then Continue;
      if D^.left  <> nil then L.Add(Format('  %s -> %s;', [NodeId(D), NodeId(D^.left)]));
      if D^.right <> nil then L.Add(Format('  %s -> %s;', [NodeId(D), NodeId(D^.right)]));
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      L.Add(Format('  %s [label="ID: %d\lAsunto: %s\lFecha: %s\lMensaje: %s\l"];',
...
var
  ...
  begin
    L.Add(Format(
      ...
      '</table>>];',
      [ Id, HTMLEscape(Remi), HTMLEscape(Asun), HTMLEscape(Fec), HTMLEscape(Trunc8(H)) ]));
  end;
  ...
  var
    ...
    curCount, k         : Integer;
    ids, idsNext        : array of string;
    hs,  hsNext         : array of string;
  ...
  begin
    ...
    begin
      L.Add(Format('  root [shape=box, style="rounded,bold", label="Hash: %s"];',
                   [HTMLEscape(Trunc8(LevelHashes[curLevel][0]))]));
      L.Add(Format('  root -> %s;', [LevelIds[curLevel][0]]));
    end;
  end;

  function IsPrivateState(const S: string): Boolean; inline;
  var U: string;
  begin
    U := UpperCase(Trim(S));
    Result := (U='PR') or (U='PV') or (U='PRIV') or (U='PRIVADO') or (U='PRIVADA');
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      // Incluye privados RECIBIDOS por el usuario (destinatario=OwnerEmail)
      // y también privados ENVIADOS por él (remitente=OwnerEmail),
      // porque algunas implementaciones incluyen ambos en “Privados”.
      if IsPrivateState(st) and
         ( (CompareText(Trim(p^.destinatario), Trim(OwnerEmail)) = 0) or
           (CompareText(Trim(p^.remitente),    Trim(OwnerEmail)) = 0) ) then
      begin
        // Hash de la hoja con campos relevantes
