...
type
  ...
  TBlock = record
    Index     : LongInt;
    Timestamp : string;  // 'dd-mm-yy::hh:nn:ss' o el formato que guardes en correo
    Data      : string;  // Texto libre (ej. Id/Asunto)
    Nonce     : LongInt;
    PrevHash  : string;
    Hash      : string;
    Next      : PBlock;  // el más nuevo está en FHead
  end;
  ...
  TBlockchain = class
  private
    FHead  : PBlock;   // bloque más nuevo
    FCount : LongInt;  // cantidad de bloques
...
var
  ...
  begin
    ...
    Inc(H[4], e); Inc(H[5], f); Inc(H[6], g); Inc(H[7], h);
  end;
...
begin
  ...
  Result :=
    ToHex(H[0]) + ToHex(H[1]) + ToHex(H[2]) + ToHex(H[3]) +
...
begin
  ...
  repeat
    ...
    Inc(nonce);
    // regla simple: hash que empiece con '190' (rápida de minar; si quieres endurecer cambia el prefijo)
  until (Length(h) >= 3) and (Copy(h,1,3) = '190');
...
begin
  ...
  try
    ...
    for i := 0 to High(arr) do
      if i = 0 then
        AddNode(Format('b%d',[i]), 'Index: 0', arr[i])
      else
        AddNode(Format('b%d',[i]), Format('Index: %d',[arr[i]^.Index]), arr[i]);
