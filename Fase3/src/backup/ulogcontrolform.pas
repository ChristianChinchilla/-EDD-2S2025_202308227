...
uses
  ...
  fpjson, Math, StrUtils; // StrUtils por SameText si lo necesitas
...
begin
  ...
  // Si no había entrada previa, registramos solo salida (queda entrada = 0)
...
begin
  ...
  btnVisualizar.Parent := pnlTop;
  btnVisualizar.Caption := 'Visualizar control';
  btnVisualizar.SetBounds(btnExport.Left + btnExport.Width + 12, 8, 180, 32);
  btnVisualizar.Anchors := [akLeft, akTop];
  btnVisualizar.OnClick := @btnVisualizarClick;

  // Grid llenando el resto
...
var
  w: Integer;
begin
  // Ajuste proporciones de columnas
  w := grid.ClientWidth;
  if w <= 0 then Exit;

  // Usuario un poco más angosto; fechas iguales
...
begin
  ...
  // Limpia filas (por si hubo menos/más registros antes)
...
begin
  ...
  try
    ...
    begin
      ...
      try
        // TStringStream UTF-8 (si tu FPC no soporta encoding, usa Create(arr.AsJSON) y WriteToFile)
...
procedure TfrmLoginLog.btnVisualizarClick(Sender: TObject);
begin
  LoadGrid;
end;
