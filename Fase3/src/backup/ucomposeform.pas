...
type
  ...
  TfrmCompose = class(TForm)
    btnEnviar: TButton;
    btnGuardarBorrador: TButton; // << NUEVO
    edtPara: TEdit;
    edtAsunto: TEdit;
    memMensaje: TMemo;
    lblPara: TLabel;
    ...
    procedure btnGuardarBorradorClick(Sender: TObject); // << NUEVO
    procedure btnVolverClick(Sender: TObject);
  private
    FRemitente: string;
  public
    procedure OpenForUser(const AEmail: string);
    procedure LoadFromDraft(const Dest, Asunto, Mensaje: string); // para editar un borrador
  end;
...
uses
  ...
  uDraftsForm;  // << NUEVO para refrescar la vista de borradores
...
begin
  ...
  if (para='') or (asu='') or (msg='') then
    begin ShowMessage('Completa destinatario, asunto y mensaje.'); Exit; end;

  // validar contacto
  if not EsContacto(FRemitente, para) then
    begin ShowMessage('El destinatario no está en tus contactos.'); Exit; end;
...
begin
  ...
  begin
    ShowMessage('No hay contenido para guardar.'); Exit;
  end;
  ...
  // << NUEVO: refrescar el formulario de borradores si está abierto (o al crearse)
