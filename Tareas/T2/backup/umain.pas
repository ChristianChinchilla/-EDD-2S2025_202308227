...
type
  TFormMain = class(TForm)
    btnLoadJson: TButton;    // "Cargar JSON"
    btnGraph: TButton;       // "Generar Gráfica"
    dlgOpen: TOpenDialog;
  ...
  private
    FRoot: PNode;
    FJsonPath: string;
    FOutDir: string; // siempre apunta a la carpeta T2/
...
begin
  ...
  // Apuntar SIEMPRE a la carpeta T2 (raíz del proyecto), incluso si el exe corre en T2/lib/...
  FOutDir := GetProjectRootFromExe; // termina con separador
...
begin
  ...
  // --- Apertura compatible con FPC/Lazarus sin joUTF8 ---
  FS := TFileStream.Create(FileName, fmOpenRead or fmShareDenyWrite);
  try
    Parser := TJSONParser.Create(FS); // <--- sin banderas
    try
      JSON := Parser.Parse;
    finally
      Parser.Free;
